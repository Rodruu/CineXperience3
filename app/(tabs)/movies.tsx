...
export default function MoviesScreen() {
  ...
  const movies = useMemo(
    () => content.filter((c) => c.tipo === "pelicula"),
    [content]
  );
  ...
  const filtered = useMemo(() => {
    ...
    return movies.filter((c) => c.titulo.toLowerCase().includes(q));
  }, [movies, search]);
        ...
        <View style={styles.headerTop}>
          ...
          {!isLoading && (
            <View style={styles.countChip}>
              <Feather name="film" size={12} color="#e50914" />
              <Text style={styles.countChipText}>{movies.length}</Text>
            </View>
          )}
        </View>

        <View style={[styles.searchBar, searchFocused && styles.searchBarFocused]}>
          <Feather name="search" size={16} color={searchFocused ? "#e50914" : "#5a5a5a"} />
          <TextInput
      ...
      {isLoading ? (
        ...
        <View style={styles.emptyState}>
          <View style={styles.emptyIconCircle}>
            <Feather name="film" size={32} color="#2a2a2a" />
          </View>
          <Text style={styles.emptyTitle}>
            {search ? "Sin resultados" : "Sin películas asignadas"}
          </Text>
          <Text style={styles.emptyDesc}>
            {search
              ? `No hay películas con "${search}"`
              : "El administrador debe asignarte películas primero."}
          </Text>
          {search ? (
            ...
            search.trim() ? (
              <View style={styles.resultsRow}>
                <Text style={styles.resultsText}>
                  ...
                  <Text style={{ color: "#e50914" }}>{search}</Text>
                </Text>
              </View>
            ) : null
          }
...
const styles = StyleSheet.create({
  ...
  titleAccent: { width: 3, height: 22, borderRadius: 2, backgroundColor: "#e50914" },
  ...
  countChipText: { color: "#e50914", fontSize: 13, fontFamily: "Inter_600SemiBold" },
  searchBar: {
    flexDirection: "row",
    alignItems: "center",
    gap: 10,
